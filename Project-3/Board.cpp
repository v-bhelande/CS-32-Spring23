...
bool Board::sow(Side s, int hole, Side& endSide, int& endHole)
{
    if ((s != NORTH && s != SOUTH))
    {
        cerr << "Chose an invalid side, returning false...\n";
        return false;
    }
...
//            if (hole < NSIDES + m_nHoles + 1 || hole > NSIDES + 2*m_nHoles - 1)
//            {
//                cerr << "Chose an invalid hole, (" << s << ", " << num_beans.size()-hole << "), returning false...\n";
//                return false;
//            }
            isNorth = true;
...
{
    if ((s != NORTH && s != SOUTH) || hole <= 0)
    {
        cerr << "Chose an invalid side or POT, (" << s << ", " << hole << "), returning false\n";
        return false;
    }
    ...
    switch (potOwner) {
        case SOUTH:
            switch (s) {
                case SOUTH:
                    ...
                    break;
                case NORTH:
                    num_beans.at(m_nHoles+1) += num_beans.at(num_beans.size()-hole);
        ...
        case NORTH:
            switch (s) {
                case SOUTH:
                    num_beans.at(0) += num_beans.at(hole);
                    cerr << "Number of beans in pot of " << potOwner << " = " << num_beans.at(POT) << endl;
                    // Remove beans from (s, hole)
                    num_beans.at(hole) = 0;
                    return true;
                    break;
